...
def event_list_create(request):
    ...
    if request.method == "GET":
        ...
        logger.info("Incoming event creation request: %s", request.data)  # Log request data
        serializer = EventSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            logger.info("Event created successfully: %s", serializer.data)  # Log success
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        logger.error("Event creation failed: %s", serializer.errors)  # Log errors
